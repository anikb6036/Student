    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div className="bg-white dark:bg-[#161618] rounded-2xl border border-slate-150/80 dark:border-white/5 p-5 shadow-sm">
          <p className="text-xs font-mono uppercase tracking-widest text-slate-500 dark:text-gray-400">Reported Average Score</p>
          <div className="flex items-baseline gap-2 mt-1.5">
            <p className="text-3xl font-serif text-slate-800 dark:text-white">{averageScore}%</p>
            <span className="text-[10px] font-bold text-amber-500 uppercase tracking-wider font-mono">Target &gt;80%</span>
          </div>
          <p className="mt-2 text-xs text-slate-400 dark:text-gray-500">Calculated over {filteredRecords.length} evaluations</p>
        </div>

        <div className="bg-white dark:bg-[#161618] rounded-2xl border border-slate-150/80 dark:border-white/5 p-5 shadow-sm">
          <p className="text-xs font-mono uppercase tracking-widest text-slate-500 dark:text-gray-400">Honor Roll Ratio</p>
          <p className="text-3xl font-serif text-slate-800 dark:text-white mt-1.5 flex items-center gap-1.5">
          ...
          <p className="mt-2 text-xs text-slate-400 dark:text-gray-500">Perfect academic performance</p>
        </div>

        <div className="bg-white dark:bg-[#161618] rounded-2xl border border-slate-150/80 dark:border-white/5 p-5 shadow-sm">
          <p className="text-xs font-mono uppercase tracking-widest text-slate-500 dark:text-gray-400">Attendance Rate</p>
          ...
          <p className="mt-2 text-xs text-slate-400 dark:text-gray-500">On-time classroom attendance</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#161618] rounded-3xl border border-slate-150/80 dark:border-white/5 shadow-sm p-6 md:p-8">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-serif italic text-amber-500 font-bold tracking-tight flex items-center gap-2">
              ...
              Student Academic Progress & Grading
            </h2>
          ...
          {currentUser.role !== 'student' && (
            <button
              ...
              className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-amber-950 rounded-xl shadow-md font-bold text-xs flex items-center gap-2 transition active:scale-95"
            >
              ...
              >
                ...
                <div className="space-y-1.5 md:col-span-1">
                  ...
                  <input
                    ...
                    className="w-full text-amber-500 cursor-pointer accent-amber-500 h-1.5 bg-slate-200 dark:bg-zinc-800 rounded-lg appearance-none"
                  />
                </div>
                ...
                <div className="md:col-span-6 flex justify-end gap-2.5 pt-2">
                  <button
                    type="submit"
                    className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-amber-950 rounded-xl text-xs font-bold shadow transition"
                  >
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6 max-w-4xl">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-8.5 pr-3.5 py-2.5 text-xs border border-slate-200 dark:border-white/5 dark:bg-[#0A0A0B] rounded-xl text-slate-900 dark:text-white focus:outline-none"
            />
          </div>
          ...
          <div className="flex items-center gap-1.5 border border-slate-200 dark:border-white/5 rounded-xl px-2.5 bg-white dark:bg-[#0A0A0B]">
            <Filter className="w-3 text-amber-500" />
            <select
              value={performanceFilter}
              onChange={e => setPerformanceFilter(e.target.value as any)}
              className="py-2.5 text-xs bg-transparent text-slate-705 dark:text-gray-300 focus:outline-none border-0"
            >
          ...
          <div className="flex items-center gap-1.5 border border-slate-200 dark:border-white/5 rounded-xl px-2.5 bg-white dark:bg-[#0A0A0B]">
            <BookOpen className="w-3 text-amber-500" />
            <select
              value={subjectFilter}
              onChange={e => setSubjectFilter(e.target.value)}
              className="py-2.5 text-xs bg-transparent text-slate-750 dark:text-gray-300 focus:outline-none border-0"
            >
        ...
        <div className="space-y-4">
          {filteredRecords.length === 0 ? (
            <div className="border border-dashed border-slate-200 dark:border-white/5 rounded-2xl p-10 text-center text-slate-400 font-mono">
            ...
            filteredRecords.map(rec => {
              ...
              return (
                <div
                  key={rec.id}
                  className={`bg-white dark:bg-[#0F0F11] p-5 rounded-2xl border border-slate-150/80 dark:border-white/5 transition hover:-translate-y-0.5 ${
                    isExcellent
                      ? '!border-amber-500/30 dark:!border-amber-500/25 ring-1 ring-amber-500/5 bg-gradient-to-r from-amber-500/5 to-[#161618]/50'
                      : ''
                  }`}
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                          {rec.subject}
                        </span>
                        ...
                        {isExcellent && (
                          <span className="text-amber-500 text-[10px] flex items-center gap-0.5 font-bold">
                            <Sparkles className="w-3 h-3 text-amber-500 animate-pulse fill-amber-500" /> Goal Met!
                          </span>
                        )}
                      </div>
